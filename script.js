...
function searchInformation() {
  document
    .getElementById('searchInputButton')
...
function addCard(movie) {
  ...
  card.setAttribute(
    'style',
    'background-image: url(' + movie.images.jpg.large_image_url + ')'
  );
...
function deleteMovie(id) {
  fetch(
    `https://se104-project-backend.du.r.appspot.com/movie?id=642110317&&movieId=${id}`,
    {
      method: 'DELETE',
    }
  )
...
function NewAnimeContent(page) {
  ...
  fetch(
    `https://api.jikan.moe/v4/anime?type=tv&&status=upcoming&&rating=pg13&&order_by=popularity&&start_date=2022-06&&page=${page}&&sfw`
  )
    .then((response) => {
      return response.json();
    })
...
function AiringAnimeContent(page) {
  ...
  fetch(
    `https://api.jikan.moe/v4/anime?status=airing&&page=${page}&&sfw&&rating=pg13`
  )
    .then((response) => {
      return response.json();
    })
